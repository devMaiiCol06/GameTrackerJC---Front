    ...
    const calculateStats = (data, context) => {
        if (context === "library") {
            return [
                ...
                {
                    value: data.filter((game) => game.gameStatus === "In Progress").length,
                    label: "In Progress"
                },
                {
                    value: data.filter((game) => game.gameStatus === "Completed").length,
                    label: "Completed"
                },
                {
                    value: data.filter((game) => game.gameStatus === "Wishlist").length,
                    label: "Wishlist"
                }
            ];
        }
    ...
    return (
        <div className={styles.StatsHeroContainer}>
            {stats.map((stat, index) => (
                <div key={index} className={styles.statCounter}>
                    <h3>{stat.value}</h3>
                    <p>{stat.label}</p>
                </div>
            ))}
